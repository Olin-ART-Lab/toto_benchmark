...
class TaskModule(nn.Module):
    ...
    def __init__(self, num_task_inputs, task_hidden_dim, interface_dim, anchor_tensor):
        ...
        self.norm_input = nn.BatchNorm1d(num_task_inputs).double()
        self.linear1 = nn.Linear(num_task_inputs, task_hidden_dim).double()
        self.linear2 = nn.Linear(task_hidden_dim, task_hidden_dim).double()
        self.linear3 = nn.Linear(task_hidden_dim, interface_dim).double()
        self.anchor = anchor_tensor.to("cuda:0")
        self.interface_dim = interface_dim

    def forward(self, task_state):
        # task_state has the size 256(batch size)*15
        task_anchor_state = torch.cat((task_state, self.anchor), 0).double()
...
class RobotQModule(nn.Module):
    ...
    def __init__(self, num_actions, num_robot_inputs, interface_dim, robot_hidden_dim):
        super(RobotQModule, self).__init__()
        self.max_action = 1.0# env_params['action_max']  # 1.0
...
class GausRobotPiModule(nn.Module):
    ...
    def __init__(self, num_actions, num_robot_inputs, interface_dim, robot_hidden_dim):
        ...
        self.action_mean = nn.Linear(35, num_actions).double()
        self.action_log_std = nn.Linear(35, num_actions).double()
        # self.dropout = nn.Dropout(0.25)
...
class QNetwork(nn.Module):
    ...
    def __init__(self, num_task_inputs, task_hidden_dim, interface_dim, num_actions, num_robot_inputs, robot_hidden_dim, anchor_tensor):
        super(QNetwork, self).__init__()

        # Q1 architecture
        self.Qtask = TaskModule(num_task_inputs, task_hidden_dim, interface_dim, anchor_tensor)
        self.Qrobot = RobotQModule(num_actions, num_robot_inputs, interface_dim, robot_hidden_dim)
...
class GausPiNetwork(nn.Module):
    def __init__(self, num_task_inputs, task_hidden_dim, interface_dim, num_actions, num_robot_inputs, robot_hidden_dim, anchor_tensor):
        ...
        self.Probot = GausRobotPiModule(num_actions, num_robot_inputs, interface_dim, robot_hidden_dim)
    ...
    def sample(self, task_state): 
        robot_state = task_state[0:7]
        if torch.isnan(task_state).any():
            breakpoint()
        mean, log_std = self.forward(task_state)
